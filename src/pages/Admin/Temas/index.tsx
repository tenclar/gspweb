...
const Temas: React.FC = () => {
  ...
  useEffect(() => {
    async function loadTemas(): Promise<void> {
      try {
        setLoading(true);
  ...
  const handleSubmit = useCallback((data: TemasPesquisaData) => {
    setArgs(data.nome);
  }, []);
      ...
      <Panel>
        <Table cellPadding="0" cellSpacing="0">
          <thead>
            <tr>
              <th style={{ width: '35px' }}>
                <LinkButton to="/ad/cadastro/temas/novo">
                  <FiPlus />
                </LinkButton>
              </th>
              <th>Nome da Tag</th>
            ...
            {temas.map((tag) => (
              <tr key={tag.id}>
                <td style={{ textAlign: 'center' }}>
                  <EditarLinkButton to={`/ad/cadastro/temas/editar/${tag.id}`}>
                    <FiRefreshCw />
                  </EditarLinkButton>
                </td>
                <td>{tag.nome}</td>
